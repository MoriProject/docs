...
4. Copy and securely store the key. Live API keys are shown only once.

The dashboard automatically issues one test API key for each account and organization. The same key remains available for retrieval from the API Keys page.
