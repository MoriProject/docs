...
Use the automatically issued test API key with the `sk_test_` prefix when you need to verify an integration without running the underlying processing services.

Test API keys:

- use the same `Authorization: Bearer` authentication as live keys;
- return mock responses for order and order-history endpoints;
- do not run Anti-AI, watermark, or AI Detection processing;
- do not call external processing services or consume credits.

Test order IDs and file URLs are synthetic. Order history, recent stats, and webhook-list requests return empty mock data. Order detail, download, URL-refresh, webhook-detail, and webhook-event reads return not-found responses. Webhook mutation requests are rejected with a test key.
