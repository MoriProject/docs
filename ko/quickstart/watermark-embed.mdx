---
title: "워터마크 삽입 빠른 시작"
description: "이미지에 보이지 않는 워터마크 삽입하기"
---

이 가이드는 이미지에 보이지 않는 워터마크를 삽입하는 과정을 안내합니다 — 주문 생성부터 워터마크가 삽입된 결과물 다운로드까지.

## 사전 준비

- BIZ MORI API 키 ([여기서 발급](https://app.bizmori.com/keys))
- 워터마크를 삽입할 이미지 파일 (`jpeg`, `jpg`, `png`, 또는 `webp`)

## 1단계: 주문 생성

워터마크 텍스트와 함께 워터마크 삽입 주문을 생성합니다.
...
<Info>
  **별도의 확인(confirm) 단계가 필요 없습니다.** Anti-AI 및 AI Detection 서비스와 달리, 워터마크 삽입은 파일 업로드가 완료되면 처리가 자동으로 시작됩니다. 업로드 후 바로 주문 상태를 확인하시면 됩니다.
</Info>
