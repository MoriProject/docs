...
<Info>
  자동 발급된 [`sk_test_` 키](/ko/test-api-keys)로 실제 워터마크 검출이나 크레딧 사용 없이 주문 흐름을 검증할 수 있습니다. 테스트 업로드는 파일 내용을 폐기하고 결정적인 검출 상태를 반환합니다. 실제 파일에서 워터마크를 추출하려면 일반 API 키를 사용하세요.
</Info>
