...
<Note>
  Presigned URL은 **1시간** 후 만료됩니다. 만료된 경우 [URL 갱신](/api-reference/orders/프리사인드-url-갱신) 엔드포인트를 사용하여 새 URL을 받으세요.
</Note>
...
// <a href={downloadUrl} download> 요소에 전달하세요 — URL은 1시간 동안 유효합니다.
...
const ACCEPT_FORMATS = 'image/jpeg, image/png, image/webp, image/tiff, image/bmp';
const MAX_FILES = 100;
  ...
  return (
    <form onSubmit={protectImages}>
      <input
        ...
        disabled={busy}
        onChange={(event) => setFiles([...event.target.files].slice(0, MAX_FILES))}
      />
