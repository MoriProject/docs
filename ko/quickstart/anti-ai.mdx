...
<Info>
  자동 발급된 [`sk_test_` 키](/ko/test-api-keys)로 실제 Anti-AI 처리를 실행하거나 크레딧을 사용하지 않고 주문 흐름을 검증할 수 있습니다. 테스트 업로드는 파일 내용을 폐기하며 다운로드 가능한 결과를 만들지 않습니다. 실제 보호 이미지를 얻으려면 일반 API 키를 사용하세요.
</Info>
