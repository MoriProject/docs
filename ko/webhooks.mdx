...
<Steps>
  <Step title="웹훅 엔드포인트 등록">
    API 또는 [BIZ MORI 대시보드](https://app.bizmori.com/webhooks)에서 웹훅 엔드포인트를 생성할 수 있습니다. API를 통해 생성하려면 [웹훅 생성](/ko/api-reference/webhooks/create-webhook) 엔드포인트를 사용합니다:
...
테스트 주문 이벤트를 받으려면 Dashboard 또는 일반 API 키로 별도의 테스트 endpoint를 등록하세요. 요청에 `isTest: true`를 지정합니다.
...
테스트 웹훅 전송은 대상 환경의 rollout에 따라 활성화됩니다. 영속 테스트 주문과 테스트 웹훅이 활성화되지 않은 환경에서는 기존 모의 응답만 반환되고 이벤트는 전송되지 않을 수 있습니다.
...
| `downloadUrl` | string | null | 결과 파일 다운로드 URL (운영 주문은 1시간 유효, 테스트 주문은 `null`) |
...
- **단일 이벤트 재전송**: [웹훅 이벤트 재전송](/api-reference/webhooks/retry-event) 엔드포인트를 사용하여 특정 실패 이벤트를 재시도합니다.
- **일괄 재전송**: [실패한 웹훅 이벤트 일괄 재전송](/api-reference/webhooks/retry-failed-events) 엔드포인트를 사용하여 특정 기간(최대 7일) 내 실패한 모든 이벤트를 재시도합니다.
...
| 단일 이벤트 재전송 | [POST /webhooks/{id}/events/{eventId}/retry](/api-reference/webhooks/retry-event) |
| 실패 이벤트 일괄 재전송 | [POST /webhooks/{id}/events/retry-failed](/api-reference/webhooks/retry-failed-events) |
