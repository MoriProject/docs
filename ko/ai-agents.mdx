...
1. `sk_test_` 키를 사용한다면 업로드·서비스별 confirm·상태 전이·가상 결과를 확인하고 실제 처리가 실행되지 않았음을 명시하세요. 실제 end-to-end 결과가 필요할 때는 일반 API 키로 주문 상태가 `complete`에 도달하는 것을 확인하세요.
2. 일반 API 키를 사용한다면 실제 결과(다운로드한 파일 또는 탐지 필드)를 확인하세요. 테스트 키를 사용한다면 결과 파일이 생성되지 않고 다운로드가 `PROCESSED_FILE_NOT_FOUND`를 반환함을 설명하세요.
...
<Info>
  요청·업로드·상태 전이·웹훅 smoke test에는 `sk_test_` 키를 사용하세요. 실제 처리나 크레딧 사용 없이 테스트 흐름을 제공하므로, 실제 완료 결과나 파일 다운로드를 확인할 때는 일반 API 키를 사용해야 합니다.
</Info>
