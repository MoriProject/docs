...
To receive events from test orders, register a separate test endpoint from the Dashboard or with a live API key. Set `isTest` to `true`:
...
- A test API key cannot create, update, delete, list, or retry webhook endpoints.
...
Test webhook delivery depends on the target environment's rollout. If the environment has not enabled persistent test orders and test webhook delivery, the test order can still use the legacy mock behavior without sending an event.
...
| `downloadUrl` | string | null | Result file download URL (valid for 1 hour for live orders; `null` for test orders) |
