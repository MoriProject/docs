...
Create a test webhook endpoint from the Dashboard or with a live API key. Set `isTest` to `true`:
...
See [Webhooks](/webhooks) for signature verification and event payloads. Test webhook delivery becomes available only after the feature is enabled in the target environment.
...
| Webhook management mutations | Return `403 AUTH_FORBIDDEN` |
