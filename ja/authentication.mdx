...
ダッシュボードでは、アカウントおよび組織ごとにテスト API キーを 1 つずつ自動的に発行します。発行されたキーは API キーページで再度確認できます。
...
- 注文作成および注文履歴 API に対してモックレスポンスを返します。
- Anti-AI、watermark、AI Detection の処理は実行されません。
- 外部処理サービスを呼び出さず、クレジットも消費しません。

テスト注文 ID とファイル URL は実際のリソースではなく仮の値です。テストキーで Webhook の変更リクエストを送信すると拒否され、一覧・参照リクエストはモックデータを返します。
