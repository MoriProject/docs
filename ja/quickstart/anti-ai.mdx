...
const { data } = await res.json();
// data.downloadUrl — 1時間有効
```

```jsx React
const { downloadUrl } = await api(`/orders/${order.orderId}/download`);
// <a href={downloadUrl} download> 要素に渡してください — この URL は1時間有効です。
...
`downloadUrl` は **1時間** 有効なプリサインド S3 URL です。この URL から保護されたファイルを直接ダウンロードしてください。
...
| `401` | 認証失敗 | API キーを確認 |
| `429` | 使用量超過 | リクエスト頻度を減らすか、プランをアップグレード |
