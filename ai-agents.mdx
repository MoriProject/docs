...
1. With an `sk_test_` key, verify the synthetic response and state that processing did not run. For an actual end-to-end result, use a live API key and show the order reaching `complete`.
2. With a live key, show me the result actually retrieved (the downloaded file, or the detection field). With a test key, explain that its URLs are synthetic and are not downloadable.
...
<Info>
  Use an `sk_test_` key for request and response smoke tests. It returns synthetic responses without running processing or consuming credits, so use a live API key when you need to verify an actual completed result.
</Info>
