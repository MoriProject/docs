...
<Note>
  Live presigned upload URLs expire after **10 minutes**. Test upload URLs carry their own authorization and do not require a Bearer header. Upload the file before the URL expires, then call the [Confirm order](/api-reference/ai-detection/confirm-order) endpoint.
</Note>
